...
public class Main {
    public static void main(String[] args) {
        ...
        cache2.put(1, 1);
        cache2.put(2, 2);
        AssertUtils.assertEqualsInteger(1, cache2.get(1));       // 返回  1
        cache2.put(3, 3);    // 该操作会使得密钥 2 作废
        AssertUtils.assertEqualsInteger(-1, cache2.get(2));       // 返回 -1 (未找到)
        cache2.put(4, 4);    // 该操作会使得密钥 1 作废
        AssertUtils.assertEqualsInteger(-1, cache2.get(1));       // 返回 -1 (未找到)
        AssertUtils.assertEqualsInteger(3, cache2.get(3));       // 返回  3
        AssertUtils.assertEqualsInteger(4, cache2.get(4));       // 返回  4
