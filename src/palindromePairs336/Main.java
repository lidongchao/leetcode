...
public class Main {
    public static void main(String[] args) {
        ...
        AssertUtils.assertEquals2DArray(ArrayUtils.to2DObject(expect_1), ArrayUtils.toObject(actual_1));
        
        int[][] expect_2 = {{0,1}, {1,0}};
        List<List<Integer>> actual_2 = solution.palindromePairs(new String[]{"bat", "tab", "cat"});
        AssertUtils.assertEquals2DArray(ArrayUtils.to2DObject(expect_2), ArrayUtils.toObject(actual_2));

        int[][] expect_3 = {{0,1}, {1,0}};
        List<List<Integer>> actual_3 = solution.palindromePairs(new String[]{"a", ""});
        AssertUtils.assertEquals2DArray(ArrayUtils.to2DObject(expect_3), ArrayUtils.toObject(actual_3));

        int[][] expect_4 = {{3,0}, {1,3}, {4,0}, {2,4}, {5,0}, {0,5}};
        List<List<Integer>> actual_4 = solution.palindromePairs(new String[]{"a", "b", "c", "ab", "ac", "aa"});
        AssertUtils.assertEquals2DArray(ArrayUtils.to2DObject(expect_4), ArrayUtils.toObject(actual_4));

        int[][] expect_5 = {{0,1}, {1,0}, {2,1}, {2,3}, {0,3}, {3,2}};
        List<List<Integer>> actual_5 = solution.palindromePairs(new String[]{"ab", "ba", "abc", "cba"});
        AssertUtils.assertEquals2DArray(ArrayUtils.to2DObject(expect_5), ArrayUtils.toObject(actual_5));
